...
# Load Data
execfile('./program/Load_Data_EMB.py')
...
#pickle.dump(NN, open('./output/Classifiers/NN_EMB.pkl', 'wb'))
#pickle.dump(rf, open('./output/Classifiers/rf_EMB.pkl', 'wb'))
...
NN = pickle.load(open('./output/Classifiers/NN_EMB.pkl', 'rb'))
rf = pickle.load(open('./output/Classifiers/rf_EMB.pkl', 'rb'))
...
for i in range(n_boot):
    boot(i, NN, X, y, marg_effects_NN, gof_measures_NN, "EMB Neural Network", cont_NN, n_inter, c_mm) #, weight=wgt)
    boot(i, rf, X, y, marg_effects_rf, gof_measures_rf, "EMB Random Forest", cont_rf, n_inter, c_mm)
...
np.savetxt("./output/EMB_importance_rf.csv", import_features, fmt="%s", delimiter=',')
np.savetxt("./output/EMB_marginal_effects_NN.csv", out_NN, fmt="%s", delimiter=',')
np.savetxt("./output/EMB_GofF_NN.csv", gof_measures_NN, fmt="%s", delimiter=',')
np.savetxt("./output/EMB_marginal_effects_rf.csv", out_rf, fmt="%s", delimiter=',')
np.savetxt("./output/EMB_GofF_rf.csv", gof_measures_rf, fmt="%s", delimiter=',')
...
if c_mm == 1:
    ...
    np.savetxt("./output/EMB_marginal_mv_NN.csv", out_MM_NN, fmt="%s", delimiter=',')
    np.savetxt("./output/EMB_marginal_mv_rf.csv", out_MM_rf, fmt="%s", delimiter=',')
